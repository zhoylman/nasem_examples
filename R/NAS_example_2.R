...
p2 = ggplot(slinky_data, aes(x = x, y = y, group = year, color = year)) +
  ...
  annotate("segment", x = -60, xend = 60, y = 0.001, yend = 0.001, arrow = arrow(length = unit(0.3, "cm"))) +
  annotate("text", x = -0, y = 0.0017, label = "Delta*t", parse = TRUE, size = 6) +
  annotate("text", x = min(slinky_data$x)+5, y = 0.0025, label = "Drier", fontface = "bold", color = "darkred", size = 5) +
  annotate("text", x = max(slinky_data$x)-10, y = 0.0025, label = "Wetter", fontface = "bold", color = "darkblue", size = 5) +
  theme(legend.position = "bottom", panel.grid.minor = element_blank())

# Map Inset
usa = st_as_sf(maps::map("state", fill = TRUE, plot = FALSE))
idaho = usa |> filter(ID == "pennsylvania")
map_inset = ggplot() +
...
title_gg <- ggdraw() + 
  draw_label("Nonstationary Drought Trends: Adams County, PA", fontface = 'bold', size = 18) +
...
final_plot_attributed <- ggdraw(final_plot) +
  draw_plot(map_inset, x = 0.78, y = 0.63, width = 0.2, height = 0.2)

# Save Final Product
ggsave('~/temp/NAS_example_2_hoylman.png', plot = final_plot_attributed, 
